...
public class LinearRegression {
	...
	private double hypothesis(double x){
		double h;
		h=theta0+theta1*x;//hypothesis calculation
		return(h);
	...
	public String currentHypothesis(){
		...
		return(theta0+" + "+theta1+"x");//display the current hypothesis 
	}
	...
	public double currentCost(){
		double cost=0;
		double temp=0;
		for (int i=0;i<nbreOfSamples;i++){// for loop that calculates the summation for the cost function
	...
	public void gradientDescent(double alpha, int numberOfSteps) {
		...
		for (int j=0;j<numberOfSteps;j++){
			...
			for (int i=0;i<len;i++){//calculates the summation for the function 
				temp0+= ((hypothesis(samples[i]))-samplesValues[i]);
				temp1+= (((hypothesis(samples[i]))-samplesValues[i])*samples[i]);
			}
			
			theta0= theta0-alpha*(2.0/(len*1.0))*temp0;
